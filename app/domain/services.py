...
def build_state(session: Session) -> Dict:
    """
    Build the complete kiosk state including all children, tasks, and progress.
    Returns all data needed to render the main kiosk display.
    
    Args:
        session: Database session
    
    Returns:
        Dict containing date, children array with tasks/progress, and daily reward text
    """
    ensure_today_initialized(session)
    current_date = today_str()
    settings = get_settings(session)
